<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:79:"/www/web/edg_leteng_net/public_html/../app/admin/view/catagory_foods/index.html";i:1573119947;s:61:"/www/web/edg_leteng_net/app/admin/view/common/_container.html";i:1571731694;}*/ ?>
<!DOCTYPE html>
<html>
				...
				<div class="row row-lg">
					<div class="col-sm-12">
						...
						<div id="CodeGoodsTableToolbar" role="group">
							<?php if(in_array('/admin/CatagoryFoods/add',session('admin.nodes')) || session('admin.role') == 1): ?>
							<button type="button" id="add" class="btn btn-primary button-margin" onclick="CodeGoods.add()">
								<i class="fa fa-plus"></i>&nbsp;添加
							</button>
							<?php endif; if(in_array('/admin/CatagoryFoods/update',session('admin.nodes')) || session('admin.role') == 1): ?>
							<button type="button" id="update" class="btn btn-primary button-margin" onclick="CodeGoods.update()">
								<i class="fa fa-edit"></i>&nbsp;修改
							</button>
							<?php endif; if(in_array('/admin/CatagoryFoods/delete',session('admin.nodes')) || session('admin.role') == 1): ?>
...
<script>
	...
	CodeGoods.class_nameFormatter = function(value,row,index) {
		return "<a style=\"color:#666\" href='"+Feng.ctxPath + '/Foods/index?class_id='+row.class_id+"'>"+value+"</a>";
	}
	...
	CodeGoods.add = function (value) {
		var index = layer.open({type: 2,title: '创建数据',area: ['800px', '100%'],fix: false, maxmin: true,content: Feng.ctxPath + '/CatagoryFoods/add/class_id/'+value});
	...
	CodeGoods.update = function (value) {
		if(value){
			var index = layer.open({type: 2,title: '编辑数据',area: ['800px', '100%'],fix: false, maxmin: true,content: Feng.ctxPath + '/CatagoryFoods/update/class_id/'+value});
		}else{
			if (this.check()) {
				var idx = '';
    ...
    {
		var ajax = new $ax(Feng.ctxPath + "/CatagoryFoods/updateSort", function (data) {
			if ('00' === data.status) {
			} else {
				Feng.error(data.msg);
			}
	...
	CodeGoods.sort = function (value,type) {
		var ajax = new $ax(Feng.ctxPath + "/CatagoryFoods/setSort", function (data) {
			if ('00' === data.status) {
	...
	CodeGoods.delete = function (value) {
		if(value){
			Feng.confirm("是否删除选中项？", function () {
				var ajax = new $ax(Feng.ctxPath + "/CatagoryFoods/delete", function (data) {
					if ('00' === data.status) {
						Feng.success(data.msg);
						CodeGoods.table.refresh();
					} else {
	...
	$(function() {
		...
		var table = new BSTable(CodeGoods.id, Feng.ctxPath+"/CatagoryFoods/index"+url,defaultColunms,100);
