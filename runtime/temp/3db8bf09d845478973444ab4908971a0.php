<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:72:"/www/web/edg_leteng_net/public_html/../app/admin/view/frament/index.html";i:1570457892;s:61:"/www/web/edg_leteng_net/app/admin/view/common/_container.html";i:1571731694;}*/ ?>
<!DOCTYPE html>
<html>
				...
				<div class="row row-lg">
					<div class="col-sm-12">
						...
						<div id="CodeGoodsTableToolbar" role="group">
							<?php if(in_array('/admin/Frament/add',session('admin.nodes')) || session('admin.role') == 1): ?>
							<button type="button" id="add" class="btn btn-primary button-margin" onclick="CodeGoods.add()">
								<i class="fa fa-plus"></i>&nbsp;添加
							</button>
							<?php endif; if(in_array('/admin/Frament/update',session('admin.nodes')) || session('admin.role') == 1): ?>
							<button type="button" id="update" class="btn btn-primary button-margin" onclick="CodeGoods.update()">
								<i class="fa fa-edit"></i>&nbsp;修改
							</button>
							<?php endif; if(in_array('/admin/Frament/delete',session('admin.nodes')) || session('admin.role') == 1): ?>
...
<script>
	...
	CodeGoods.add = function (value) {
		...
		var index = layer.open({type: 2,title: '创建数据',area: ['800px', '600px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Frament/add'+url});
	...
	CodeGoods.update = function (value) {
		if(value){
			var index = layer.open({type: 2,title: '编辑数据',area: ['800px', '600px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Frament/update/frament_id/'+value});
		}else{
			if (this.check()) {
				var idx = '';
				...
				var index = layer.open({type: 2,title: '编辑数据',area: ['800px', '600px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Frament/update/frament_id/'+idx});
				this.layerIndex = index;
			}
		}
	}
	...
	CodeGoods.delete = function (value) {
		if(value){
			Feng.confirm("是否删除选中项？", function () {
				var ajax = new $ax(Feng.ctxPath + "/Frament/delete", function (data) {
					if ('00' === data.status) {
	...
	$(function() {
		...
		var url = location.search;
		var table = new BSTable(CodeGoods.id, Feng.ctxPath+"/Frament/index"+url,defaultColunms,20);
