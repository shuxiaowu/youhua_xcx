<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:70:"/www/web/edg_leteng_net/public_html/../app/admin/view/extend/info.html";i:1570457892;s:61:"/www/web/edg_leteng_net/app/admin/view/common/_container.html";i:1571731694;}*/ ?>
<!DOCTYPE html>
<html>
		...
		<div class="form-horizontal" id="CodeInfoForm">
				...
				<!-- form start -->
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							...
							<input name="type" value="1" lay-filter="hope"  type="radio" <?php if($info['type'] == '1'): ?>checked<?php endif; ?> title="内容拓展模型">
							<input name="type" value="2" lay-filter="hope"  type="radio" <?php if($info['type'] == '2'): ?>checked<?php endif; ?> title="表单模型">
						</div>
					</div>
					<div class="form-group" id="action">
						...
						<div class="col-sm-9">
							...
							<input name="action" value="add" type="checkbox" <?php if(in_array('add',explode(',',$info['action']))): ?>checked<?php endif; ?> title="添加">
							<input name="action" value="update" type="checkbox" <?php if(in_array('update',explode(',',$info['action']))): ?> checked<?php endif; ?> title="修改">
							<input name="action" value="delete" type="checkbox" <?php if(in_array('delete',explode(',',$info['action']))): ?> checked<?php endif; ?> title="删除">
							<input name="action" value="view" type="checkbox" <?php if(in_array('view',explode(',',$info['action']))): ?> checked<?php endif; ?> title="查看数据">
							<input name="action" value="dumpData" type="checkbox" <?php if(in_array('dumpData',explode(',',$info['action']))): ?> checked<?php endif; ?> title="数据导出">
							<input name="action" value="importData" type="checkbox" <?php if(in_array('importData',explode(',',$info['action']))): ?> checked<?php endif; ?> title="数据导入">
						</div>
					</div>
					<div class="form-group" id="sorttype">
						<label class="col-sm-2 control-label">默认排序：</label>
						<div class="col-sm-9">
							<input type="text" id="orderby" value="<?php echo $info['orderby']; ?>" name="orderby" class="form-control" placeholder="默认排序为主键倒序 data_id desc">
						</div>
					</div>
					<div class="form-group">
			...
			<div class="row btn-group-m-t">
				<div class="col-sm-9 col-sm-offset-1">
					<button type="button" class="btn btn-primary" onclick="<?php if($info['extend_id'] != ''): ?>CodeInfoDlg.update()<?php else: ?>CodeInfoDlg.add()<?php endif; ?>" id="ensure">
