<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:70:"/www/web/edg_leteng_net/public_html/../app/admin/view/foods/index.html";i:1573209935;s:61:"/www/web/edg_leteng_net/app/admin/view/common/_container.html";i:1571731694;}*/ ?>
<!DOCTYPE html>
<html>
...
<script>
	...
	CodeGoods.upsort = function (content_id, sortid) {
		var ajax = new $ax(Feng.ctxPath + "/Foods/updateSort", function (data) {
			if ('00' === data.status) {
			} else {
				Feng.error(data.msg);
			}
	...
	CodeGoods.add = function (value) {
		...
		var index = layer.open({ type: 2, title: '创建数据', area: ['800px', '100%'], fix: false, maxmin: true, content: Feng.ctxPath + '/Foods/add' + url });
	...
	CodeGoods.update = function (value) {
		if (value) {
			var index = layer.open({ type: 2, title: '编辑数据', area: ['800px', '100%'], fix: false, maxmin: true, content: Feng.ctxPath + '/Foods/update/content_id/' + value });
		} else {
			if (this.check()) {
				var idx = '';
				...
				var index = layer.open({ type: 2, title: '编辑数据', area: ['800px', '100%'], fix: false, maxmin: true, content: Feng.ctxPath + '/Foods/update/content_id/' + idx });
				this.layerIndex = index;
			}
		}
	}
	...
	CodeGoods.delete = function (value) {
		if (value) {
			Feng.confirm("是否删除选中项？", function () {
				var ajax = new $ax(Feng.ctxPath + "/Foods/delete", function (data) {
					if ('00' === data.status) {
	...
	CodeGoods.move = function (value) {
		if (this.check()) {
			...
			Feng.confirm("是否移动选中项？", function () {
				var ajax = new $ax(Feng.ctxPath + "/Foods/move", function (data) {
					if ('00' === data.status) {
	...
	CodeGoods.setStatus = function (value) {
		...
		if (this.check()) {
			...
			Feng.confirm("是否移动选中项？", function () {
				var ajax = new $ax(Feng.ctxPath + "/Foods/setStatus", function (data) {
					if ('00' === data.status) {
						Feng.success(data.message);
						CodeGoods.table.refresh();
					} else {
	...
	CodeGoods.position = function (type) {
		if (this.check()) {
			...
			Feng.confirm("是否操作选中项？", function () {
				var ajax = new $ax(Feng.ctxPath + "/Foods/setPosition", function (data) {
					if ('00' === data.status) {
						Feng.success(data.message);
						CodeGoods.table.refresh();
					} else {
	...
	CodeGoods.delPosition = function (position_id, content_id) {
		Feng.confirm("确定删除推荐位吗", function () {
			var ajax = new $ax(Feng.ctxPath + "/Foods/delPosition", function (data) {
				if ('00' === data.status) {
					Feng.success(data.message);
					CodeGoods.table.refresh();
				} else {
	...
	$(function () {
		...
		var url = location.search;
		var table = new BSTable(CodeGoods.id, Feng.ctxPath + "/Foods/index/" + url, defaultColunms, 20);
