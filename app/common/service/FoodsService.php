...
class FoodsService
{
    ...
    public static function loadList($where, $field, $limit, $orderby)
    {
        try {
            ...
            $list = model('Foods')->loadList($where, $field, $limit, $orderby);
            $count = model('Foods')->countList($where);
        } catch (\Exception $e) {
    ...
    public static function loadRelateList($where, $field, $limit, $orderby)
    {
        try {
            ...
            $result = model('Foods')->loadRelateList($where, $field, $limit, $orderby);
        } catch (\Exception $e) {
            throw new \Exception($e->getMessage());
        }
    ...
    public static function saveData($type, $data)
    {
        try {
            ...
            if ($type == 'add') {
                ...
                $reset = model('Foods')->createData($data);
                if ($reset) {
                    model('Foods')->edit(['content_id'=>$reset,'sortid'=>$reset]);
                }
    ...
    public static function saveExtData($data)
    {
        ...
        $fieldsetInfo = model("CatagoryFoods")->getInfo($classId);
    ...
    {
        try {
            $reset = model('Foods')->editWhere($where, $data);
        } catch (\Exception $e) {
            throw new \Exception($e->getMessage());
        }
    ...
    {
        try {
            $contentInfo = db('foods')->alias('a')->join('catagory b', 'a.class_id=b.class_id')->where(['a.content_id'=>$where['content_id']])->find();
            ...
            $reset = model('Foods')->delete($where);
        } catch (\Exception $e) {
            throw new \Exception($e->getMessage());
        }
    ...
    {
        $list = db("Catagory_foods")->where(['pid'=>$pid])->field('class_id,pid,class_name,class_id as childs')->select();
        foreach ($list as $key=>$val) {
            $sublist = db("Catagory_foods")->where(['pid'=>$val['class_id']])->field('class_id,pid,class_name,class_id as childs')->select();
            if ($sublist) {
                $childs = model('index/CatagoryFoods')->getSubClassId($val['class_id']);
