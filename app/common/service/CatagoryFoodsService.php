...
class CatagoryFoodsService
{
	...
	public static function delete($where){
		...
		try{	
			$reset = model('CatagoryFoods')->delete($where);
		}catch(\Exception $e){
	...
	public static function saveData($type,$data){
		...
		try{
			...
			if($type == 'add'){
				...
				$reset = model('CatagoryFoods')->createData($data);
				if($reset){
					model('CatagoryFoods')->edit(['class_id'=>$reset,'sortid'=>$reset]);
				}
			}elseif($type == 'edit'){
				...
				$reset = model('CatagoryFoods')->edit($data);
	...
	public static function setSort($class_id,$type){
		$data = model("CatagoryFoods")->getInfo($class_id);

		if($type == 1){
			$map['sortid']  = array('lt',$data['sortid']);
			$map['pid'] = $data['pid'];
			$info = model("CatagoryFoods")->getWhereInfo($map,$orderby='sortid desc');
		}else{
			$map['sortid']  = array('gt',$data['sortid']);
			$map['pid'] = $data['pid'];
			$info = model("CatagoryFoods")->getWhereInfo($map,$orderby='sortid asc');
		}
		...
		if(!$info)
		{
		   ...
		   try{
				...
				$r1 = db("catagory_foods")->where($selfMap)->setField(array('sortid'=>$info['sortid']));
				$r2 = db("catagory_foods")->where($targetMap)->setField(array('sortid'=>$data['sortid']));
		   }catch(\Exception $e){
				throw new \Exception($e->getMessage());
			}
		   return true;
