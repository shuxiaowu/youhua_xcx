...
class Check
{
    public function appBegin()
    {
        ...
        if (!$sign) {
            echo json_encode(["code" => -10001,'msg'=>"sign参数异常"]);
            die;
        }
        if (!$timestamp) {
            echo json_encode(["code" => -10002,'msg'=>"timestamp参数异常"]);
            die;
        }
        if (abs(strtotime($timestamp) - time()) >=300) {
            echo json_encode(["code" => -10003,'msg'=>"当前请求请求超时"]);
            die;
        }
        $input = input();
        ...
        if ($signServer != $sign) {
            echo json_encode(["code" => -10004,'msg'=>"非法攻击,参数被篡改"]);
            die;
        }
    }
