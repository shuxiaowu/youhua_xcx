...
class CatagoryFoods extends Model
{
    ...
    public function loadList($where = [], $classId=0){
        $data =	db("catagory_foods")->field("class_id,class_name,pid,jumpurl,filepath,filename")->select();
	...
	public function getTopBigInfo($classId){
		$crumb = $this->loadCrumb($classId);
		return model('common/CatagoryFoods')->getInfo($crumb[0]['class_id']);
	}
