...
use app\common\service\CatagoryFoodsService;

class CatagoryFoods extends Admin {
	...
	function index(){
		if (!$this->request->isAjax()){
			...
			$list = model('CatagoryFoods')->loadRelateList($where = [],'a.*,b.title as extend_name','sortid asc');
			$list = formartList(['class_id', 'pid', 'class_name','class_name'],$list);
			$data['rows']  = $list;
			$data['total'] = model('CatagoryFoods')->countList($where = []);
	...
	function add(){
		if (!$this->request->isPost()){
			...
			$info = model('CatagoryFoods')->getInfo($class_id);
			...
			$this->assign('tpList',CatagoryFoodsService::tplList(config('default_themes')));
			$this->assign('info',$data);
			return $this->fetch('info');
		}else{
			$data = input('post.');
			try {
				$res = CatagoryFoodsService::saveData('add',$data);
			} catch (\Exception $e) {
	...
	function update(){
		if (!$this->request->isPost()){
			...
			$data = input('post.');
			try {
				$res = CatagoryFoodsService::saveData('edit',$data);
			} catch (\Exception $e) {
				$this->error($e->getMessage());
			}
	...
	function setSort(){
		...
		try{
			CatagoryFoodsService::setSort($class_id,$type);
		}catch(\Exception $e){
			exit(json_encode(array('status'=>'02','msg'=>$e->getMessage())));
		}
		echo json_encode(array('status'=>'00','msg'=>'操作成功'));
		...
		try{
			...
			if($idx){
				foreach($idx as $id){
					...
					if(!$info){
						$res = CatagoryFoodsService::delete($where);
					}else{
